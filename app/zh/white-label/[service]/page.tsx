...
export async function generateMetadata(
  ...
  const url = `https://aenfinite.com/zh/white-label/${service}/`;
...
<div class="header" style="min-height:40vh;display:flex;align-items:center;padding:120px 24px 45px;">
  <div style="max-width:1050px;margin:0 auto;width:100%;">
    ...
    <p style="margin-top:24px;">
      <a href="/zh/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 30px;border-radius:8px;font-weight:600;font-size:16px;text-decoration:none;">Request Wholesale 机构 Rates</a>
    </p>
  </div>
</div>
...
<section ${S}>
  <div style="background:rgba(34,123,243,0.06);border-left:4px solid #227bf3;border-radius:0 12px 12px 0;padding:24px 28px;margin-bottom:32px;">
    <h2 style="font-size:20px;font-weight:700;margin:0 0 8px;color:#227bf3;">The 机构 Scaling Bottleneck</h2>
...
<section ${S}>
  <h2 ${H2}>What We Deliver for Your 机构</h2>
...
<section ${S}>
  <h2 ${H2}>Wholesale 机构 Pricing &amp; Turnaround</h2>
  <p ${P}>${svc.pricing}</p>
...
<section ${S}>
  <h2 ${H2}>Explore 其他 White-Label Capabilities</h2>
  <div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:16px;">
    ${svc.relatedServices.map(slug => {
      const rel = DATA[slug];
      if (!rel) return '';
      return `<a href="/zh/white-label/${slug}/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.25);border-radius:24px;text-decoration:none;color:inherit;font-size:15px;font-weight:500;">${rel.icon} ${rel.name} &rsaquo;</a>`;
    }).join('')}
    <a href="/zh/white-label/" style="padding:10px 18px;border:1px solid #227bf3;border-radius:24px;text-decoration:none;color:#227bf3;font-size:15px;font-weight:600;">查看全部 White-Label 服务 &rsaquo;</a>
  </div>
</section>
...
<section ${S} style="padding-bottom:50px;text-align:center;">
  <div style="border:1px solid rgba(128,128,128,0.2);border-radius:14px;padding:36px 20px;">
    ...
    <p style="font-size:16px;opacity:0.85;margin:0 auto 20px;max-width:550px;">发送 us your project scope for a guaranteed 24-hour wholesale price quote and delivery timeline.</p>
    <a href="/zh/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 28px;border-radius:8px;font-weight:600;text-decoration:none;">Submit a Project for Scoping &rsaquo;</a>
  </div>
</section>
`;
}
    ...
    '@graph': [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'White-Label 服务', item: 'https://aenfinite.com/white-label/' },
