...
export async function generateMetadata(
  ...
  const url = `https://aenfinite.com/zh/ai-solutions/${solution}/`;
...
<div class="header" style="min-height:40vh;display:flex;align-items:center;padding:120px 24px 45px;">
  <div style="max-width:1050px;margin:0 auto;width:100%;">
    ...
    <p style="margin-top:24px;">
      <a href="/zh/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 30px;border-radius:8px;font-weight:600;font-size:16px;text-decoration:none;">Schedule a Live AI Demo</a>
    </p>
  </div>
</div>
...
<section ${S}>
  <h2 ${H2}>Transparent Setup &amp; 维护 Pricing</h2>
  <p ${P}>${sol.pricing}</p>
...
<section ${S}>
  <h2 ${H2}>Pair With Our Industry 网页设计</h2>
  <p ${P}>Upgrade your complete digital presence with our purpose-built industry website for your vertical:</p>
  <div style="margin-top:14px;">
    <a href="/zh/industries/${sol.relatedIndustry}/" style="display:inline-block;padding:12px 24px;border:1px solid #227bf3;border-radius:8px;text-decoration:none;color:#227bf3;font-weight:600;font-size:16px;">
      View ${sol.name.replace('AI for ', '').replace('AI Receptionist for ', '').replace('AI Intake for ', '').replace('AI Assistant for ', '').replace('AI Ordering for ', '')} 网页设计 &rsaquo;
    </a>
  </div>
</section>
...
<section ${S} style="padding-bottom:50px;text-align:center;">
  <div style="border:1px solid rgba(128,128,128,0.2);border-radius:14px;padding:36px 20px;">
    ...
    <p style="font-size:16px;opacity:0.85;margin:0 auto 20px;max-width:550px;">联系我们 our engineering team to hear a test call and receive a custom workflow architecture diagram.</p>
    <a href="/zh/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 28px;border-radius:8px;font-weight:600;text-decoration:none;">Book Your Live AI Demo &rsaquo;</a>
  </div>
</section>
`;
}
    ...
    '@graph': [
      {
        ...
        '@id': `${url}#service`,
        name: sol.title,
        serviceType: `AI Voice & Chat 自动化 for ${sol.name}`,
        description: sol.metaDesc,
