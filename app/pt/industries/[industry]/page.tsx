...
export async function generateMetadata(
  ...
  const url = `https://aenfinite.com/pt/industries/${industry}/`;
...
<div class="header" style="min-height:40vh;display:flex;align-items:center;padding:120px 24px 45px;">
  <div style="max-width:1050px;margin:0 auto;width:100%;">
    ...
    <p style="margin-top:24px;">
      <a href="/pt/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 30px;border-radius:8px;font-weight:600;font-size:16px;text-decoration:none;">Schedule a Discovery Call</a>
    </p>
  </div>
</div>
...
<section ${S}>
  <h2 ${H2}>Explore Outro Industry Solutions</h2>
  ...
  <div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:16px;">
    ${ind.relatedIndustries.map(slug => {
      const rel = DATA[slug];
      if (!rel) return '';
      return `<a href="/pt/industries/${slug}/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.25);border-radius:24px;text-decoration:none;color:inherit;font-size:15px;font-weight:500;">${rel.icon} ${rel.name} &rsaquo;</a>`;
    }).join('')}
    <a href="/pt/industries/" style="padding:10px 18px;border:1px solid #227bf3;border-radius:24px;text-decoration:none;color:#227bf3;font-size:15px;font-weight:600;">Ver Tudo Industries &rsaquo;</a>
  </div>
</section>
...
<section ${S} style="padding-bottom:50px;text-align:center;">
  <div style="border:1px solid rgba(128,128,128,0.2);border-radius:14px;padding:36px 20px;">
    ...
    <p style="font-size:16px;opacity:0.85;margin:0 auto 20px;max-width:550px;">Contato our Denver team for a complimentary audit and fixed-price scope proposal.</p>
    <a href="/pt/contact/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 28px;border-radius:8px;font-weight:600;text-decoration:none;">Book Your Consulta Gratuita &rsaquo;</a>
  </div>
</section>
`;
}
    ...
    '@graph': [
      {
        ...
        '@id': `${url}#service`,
        name: ind.title,
        serviceType: `Web Design for ${ind.name}`,
        description: ind.metaDesc,
