...
export default async function AuthorPage({ params }: { params: Promise<{ name: string }> }) {
  const { name } = await params;
  const authorName = name.charAt(0).toUpperCase() + name.slice(1);
  const url = `https://aenfinite.com/fr/author/${name}/`;
  ...
  const pageBody = `
    ...
    <section ${S}>
      <h2 style="font-size:26px;font-weight:700;margin-bottom:20px;">À propos ${authorName}</h2>
      <p ${P}>${authorName} is an integral part of the Aenfinite team, specializing in creating high-conversion web platforms, AI automation systems, and enterprise-grade software. With a focus on performance and robust engineering, ${authorName} helps businesses navigate transformation digitale and scale efficiently.</p>
