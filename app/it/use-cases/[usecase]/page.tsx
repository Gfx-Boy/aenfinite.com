...
export async function generateMetadata({ params }: { params: Promise<{ usecase: string }> }): Promise<Metadata> {
  ...
  if (!uc) return {};
  const url = `https://aenfinite.com/it/use-cases/${uc.slug}/`;
...
function buildPage(uc: UseCaseItem): string {
  ...
  const industryLinks = uc.relatedIndustries.map(slug =>
    `<a href="/it/industries/${slug}/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.2);border-radius:8px;text-decoration:none;color:inherit;font-size:15px;">Web Design for ${slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</a>`
  ).join('\n');

  const aiLinks = uc.relatedAiSolutions.map(slug =>
    `<a href="/it/ai-solutions/${slug}/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.2);border-radius:8px;text-decoration:none;color:inherit;font-size:15px;">AI Solutions for ${slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</a>`
  ).join('\n');
...
<div class="header aen-dark-mode" style="min-height:42vh;display:flex;align-items:center;padding:130px 24px 50px;">
  <div style="max-width:1100px;margin:0 auto;width:100%;">
    <p style="font-size:14px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;opacity:0.6;margin:0 0 12px;">
      <a href="/it/use-cases/" style="text-decoration:none;color:inherit;">Use Cases</a> &rsaquo; ${uc.name}
    </p>
...
<section ${S}>
  ...
  <p style="margin-top:20px;font-size:15px;opacity:0.7;">Don't see your tool? We integrate with anything that has an API. <a href="/it/tools/" style="color:#227bf3;">See our full tech stack &rsaquo;</a></p>
</section>
...
<section ${S}>
  ...
  <div style="display:flex;flex-wrap:wrap;gap:12px;">
    ${industryLinks}
    ${aiLinks}
    <a href="/it/use-cases/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.2);border-radius:8px;text-decoration:none;color:inherit;font-size:15px;">Vedi Tutto Use Cases</a>
  </div>
</section>
` : ''}
    ...
    '@graph': [
      {
        ...
        '@id': `${url}#service`,
        name: uc.name,
        serviceType: 'AI Automazione System',
        description: uc.metaDesc,
