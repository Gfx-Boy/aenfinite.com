...
export async function generateMetadata({ params }: { params: Promise<{ tool: string }> }): Promise<Metadata> {
  ...
  if (!item) return {};
  const url = `https://aenfinite.com/it/tools/${item.slug}/`;
...
function buildPage(item: ToolItem): string {
  ...
  const useCaseLinks = item.useCases.map(slug =>
    `<a href="/it/use-cases/${slug}/" style="padding:10px 18px;border:1px solid rgba(128,128,128,0.2);border-radius:8px;text-decoration:none;color:inherit;font-size:15px;">${slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</a>`
  ).join('\n');
...
<div class="header aen-dark-mode" style="min-height:42vh;display:flex;align-items:center;padding:130px 24px 50px;">
  <div style="max-width:1100px;margin:0 auto;width:100%;">
    <p style="font-size:14px;font-weight:600;text-transform:uppercase;letter-spacing:1.5px;opacity:0.6;margin:0 0 12px;">
      <a href="/it/tools/" style="text-decoration:none;color:inherit;">Tecnologia Stack</a> &rsaquo; ${item.name}
    </p>
    ...
    <div style="display:flex;gap:16px;flex-wrap:wrap;margin-top:28px;">
      <a href="/it/audit/" class="button" style="display:inline-block;background:#227bf3;color:#fff;padding:14px 32px;border-radius:8px;font-weight:600;text-decoration:none;">Get Expert ${item.name} Help &rsaquo;</a>
    </div>
  </div>
</div>
