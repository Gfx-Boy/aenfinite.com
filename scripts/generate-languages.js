...
// Import comprehensive translations for all languages (from JSON files)
const allTranslations = require('./translations');
...
// Merge comprehensive translations from JSON files for ALL languages
for (const [langCode, langConfig] of Object.entries(LANGUAGES)) {
  if (allTranslations[langCode]) {
    langConfig.translations = { ...langConfig.translations, ...allTranslations[langCode] };
    console.log(`  Merged ${Object.keys(allTranslations[langCode]).length} comprehensive ${langConfig.name} translations`);
  }
}
 ...
 * target language equivalents. Uses word-boundary-aware matching to
 * prevent partial-match corruption (e.g., "Work" inside "Workflow").
 */
function translateContent(content, lang) {
  ...
  // ── STEP 1: Protect attributes that must NOT be translated ──
  // Extract src="...", srcset="...", data-src="...", action="...", style="..."
  // and replace with placeholders to prevent corruption of file paths/URLs
  const protectedValues = [];
  result = result.replace(
    /((?:src|srcset|data-src|data-project-group|action|style|background)\s*=\s*)(["'])((?:(?!\2).)*)\2/gi,
    (match, prefix, quote, value) => {
      const idx = protectedValues.length;
      protectedValues.push({ prefix, quote, value });
      return `__PROTECTED_ATTR_${idx}__`;
    }
  );

  // Also protect <image href="..."> (SVG image elements — not navigation links)
  result = result.replace(
    /(<image\s[^>]*?href\s*=\s*)(["'])((?:(?!\2).)*)\2/gi,
    (match, prefix, quote, value) => {
      const idx = protectedValues.length;
      protectedValues.push({ prefix, quote, value });
      return `__PROTECTED_ATTR_${idx}__`;
  ...
  // Sort translations by length (longest first) to prevent shorter phrases
  // from breaking longer ones (e.g., "Design" before "Web Design")
  ...
  for (const eng of sortedKeys) {
    ...
    if (!translated || eng === translated) continue;

    // Escape special regex characters in the source string
    const escaped = eng.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

    // Use negative lookbehind/lookahead ($\w) to prevent matching inside larger words
    // e.g., prevents "Work" from matching inside "Workflow" or "Network"
    try {
      const pattern = new RegExp(`(?<!\\w)${escaped}(?!\\w)`, 'g');
      result = result.replace(pattern, translated);
    } catch (e) {
      // Fallback to split/join for complex patterns that fail regex
      result = result.split(eng).join(translated);
    }
  }

  // ── STEP 3: Restore protected attributes ──
  result = result.replace(/__PROTECTED_ATTR_(\d+)__/g, (match, idxStr) => {
    const idx = parseInt(idxStr, 10);
    const { prefix, quote, value } = protectedValues[idx];
...
function transformMetadata(content, lang, pagePath) {
  ...
  // Build alternates.languages for per-page hreflang
  const languagesObj = {};
  ...
  // Replace the ENTIRE alternates block (handles nested braces from languages object)
  ...
  // Replace OG url (only the top-level "url" property, NOT image URLs)
  // Match "url":"https://aenfinite.com/..." but NOT inside "images":[{"url":...}]
  result = result.replace(
    /("siteName"|"type").*?"url"\s*:\s*"https:\/\/aenfinite\.com\/[^"]*"/g,
    (match) => match // don't change siteName/type context matches
  );
  // Only replace the OG url that appears BEFORE siteName (the main OG url)
  result = result.replace(
    /(openGraph:\s*\{[^}]*?"url"\s*:\s*)"https:\/\/aenfinite\.com\/[^"]*"/,
    `$1"${langUrl}"`
  );
...
/**
 * Prefix all internal links with the language code.
 * Converts href="/services/" to href="/fr/services/" etc.
 * Handles both absolute and relative internal links.
 */
function prefixInternalLinks(content, lang) {
  let result = content;

  // Match href="/..." (absolute internal links) — but NOT external URLs or anchors
  // Don't prefix links that already have a language prefix
  // Don't prefix static asset paths (wp-content, images, css, js, fonts, etc.)
  const langCodes = Object.keys(LANGUAGES).join('|');
  const staticPrefixes = 'wp-content|wp-includes|wp-admin|static|assets|images|css|js|fonts|favicon|_next';

  // Use new RegExp() since we need variable interpolation for language codes
  // href="/services/" → href="/fr/services/"
  // But NOT href="/fr/services/" (already prefixed)
  // But NOT href="/wp-content/..." (static assets)
  const linkPattern = new RegExp(
    `href=(["'])\\/(?!(${langCodes})\\/|\\/|${staticPrefixes})([^"']*?)\\1`,
    'g'
  );
  result = result.replace(linkPattern, function(match, quote, _, urlPath) {
    // Skip file extensions that indicate static assets
    if (/\.(svg|png|jpg|jpeg|gif|webp|avif|ico|css|js|woff2?|ttf|eot|pdf|mp4|webm)$/i.test(urlPath)) {
      return match;
    }
    return `href=${quote}/${lang}/${urlPath}${quote}`;
  });

  // Also handle href="/" (homepage) — must be exact match
  result = result.replace(
    /href=(["'])\/\1/g,
    function(match, quote) {
      return `href=${quote}/${lang}/${quote}`;
    }
  );

  // Fix double-prefixing that may occur from regex overlap
  const doublePrefixPattern = new RegExp(`/${lang}/${lang}/`, 'g');
  result = result.replace(doublePrefixPattern, `/${lang}/`);

  return result;
}

/**
 * Fix Schema/JSON-LD content for the target language.
 * - Changes inLanguage from "en-US" to target locale
 * - Translates day names in openingHours
 * - Updates availableLanguage
 */
function fixSchemaLanguage(content, lang) {
  let result = content;
  const cfg = LANGUAGES[lang];
  ...
  const localeMap = {
    ...
    ar: 'ar-SA', pt: 'pt-BR', zh: 'zh-CN', hi: 'hi-IN',
    nl: 'nl-NL', ja: 'ja-JP', ko: 'ko-KR'
  };

  const targetLocale = localeMap[lang] || lang;

  // Replace inLanguage in JSON-LD
  result = result.replace(/"inLanguage"\s*:\s*"en-US"/g, `"inLanguage":"${targetLocale}"`);
  result = result.replace(/"inLanguage"\s*:\s*"en"/g, `"inLanguage":"${targetLocale}"`);
  result = result.replace(/inLanguage:\s*["']en-US["']/g, `inLanguage: "${targetLocale}"`);
  result = result.replace(/inLanguage:\s*["']en["']/g, `inLanguage: "${targetLocale}"`);

  // Translate day names in openingHoursSpecification / openingHours
  ...
  if (dayTranslations[lang]) {
    for (const [engDay, transDay] of Object.entries(dayTranslations[lang])) {
      // Only replace day names within Schema context (near openingHours)
      result = result.replace(new RegExp(`"${engDay}"`, 'g'), `"${transDay}"`);
    }
  }
...
/**
 * Translate keywords in metadata for SEO
 */
function translateKeywords(content, lang) {
  const keywordTranslations = {
    es: {
      ...
      'digital marketing agency Denver': 'agencia de marketing digital Denver',
      'UI UX design': 'diseño UI UX',
      'custom web development': 'desarrollo web personalizado',
      'SEO services': 'servicios SEO',
      'app development': 'desarrollo de aplicaciones',
      'e-commerce development': 'desarrollo de e-commerce',
      'digital transformation': 'transformación digital',
      'creative agency': 'agencia creativa',
      'web development company': 'empresa de desarrollo web',
      'Denver web design': 'diseño web Denver',
      'branding services': 'servicios de branding',
      'logo design services': 'servicios de diseño de logotipo',
    },
    fr: {
      'digital agency': 'agence digitale',
      'web design agency': 'agence de conception web',
      'branding agency': 'agence de branding',
      'digital marketing agency Denver': 'agence de marketing digital Denver',
      'UI UX design': 'design UI UX',
      'custom web development': 'développement web sur mesure',
      'SEO services': 'services SEO',
      'app development': 'développement d\'applications',
      ...
      'web development company': 'société de développement web',
      'Denver web design': 'conception web Denver',
      'branding services': 'services de branding',
      'logo design services': 'services de création de logo',
    },
    de: {
      'digital agency': 'Digitalagentur',
      'web design agency': 'Webdesign-Agentur',
      'branding agency': 'Branding-Agentur',
      'digital marketing agency Denver': 'Agentur für digitales Marketing Denver',
      'UI UX design': 'UI UX Design',
      'custom web development': 'individuelle Webentwicklung',
      'SEO services': 'SEO-Dienstleistungen',
      'app development': 'App-Entwicklung',
    ...
    it: {
      'digital agency': 'agenzia digitale',
      'web design agency': 'agenzia di web design',
      'branding agency': 'agenzia di branding',
      'SEO services': 'servizi SEO',
      'app development': 'sviluppo app',
    ...
    ar: {
      'digital agency': 'وكالة رقمية',
      'web design agency': 'وكالة تصميم مواقع',
      'branding agency': 'وكالة علامات تجارية',
      'SEO services': 'خدمات تحسين محركات البحث',
    ...
    nl: {
      'digital agency': 'digitaal bureau',
      'web design agency': 'webdesign bureau',
      'SEO services': 'SEO diensten',
      'digital transformation': 'digitale transformatie',
    },
    ja: {
      'digital agency': 'デジタルエージェンシー',
      'web design agency': 'Webデザインエージェンシー',
      'SEO services': 'SEOサービス',
      'digital transformation': 'デジタルトランスフォーメーション',
    },
    ko: {
      'digital agency': '디지털 에이전시',
      'web design agency': '웹 디자인 에이전시',
      'SEO services': 'SEO 서비스',
      'digital transformation': '디지털 전환',
    },
  };

  if (!keywordTranslations[lang]) return content;

  let result = content;
  const kwTrans = keywordTranslations[lang];

  // Find keywords arrays/strings in metadata and translate them
  // Match patterns like: keywords: ["...", "..."] or keywords: "..."
  result = result.replace(
    /(keywords\s*:\s*)(\[[^\]]*\]|"[^"]*")/g,
    (match, prefix, kwBlock) => {
      let translated = kwBlock;
      // Sort by length to prevent partial matches
      const sorted = Object.keys(kwTrans).sort((a, b) => b.length - a.length);
      for (const eng of sorted) {
        const trans = kwTrans[eng];
        if (trans) {
          translated = translated.split(eng).join(trans);
        }
      }
...
function generateLangPage(sourcePath, langCode, pagePath) {
  ...
  // 1. Translate content strings (word-boundary-aware)
  result = translateContent(result, langCode);

  // 2. Prefix internal links with language code
  result = prefixInternalLinks(result, langCode);

  // 3. Fix Schema/JSON-LD language attributes
  result = fixSchemaLanguage(result, langCode);

  // 4. Translate keywords in metadata
  result = translateKeywords(result, langCode);

  // 5. Transform metadata (canonical, OG URLs)
  result = transformMetadata(result, langCode, pagePath);

  // 6. Add language indicator comment at top
